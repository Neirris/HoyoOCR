...
class DatasetGenerator:
    ...
    def trim_transparent_pixels(self, image, mask, boxes):
        ...
        if len(image_np.shape) == 2:  # Grayscale
            ...
            non_white = np.where(
                (image_np[:, :, :3] < 255).any(axis=2) | (image_np[:, :, 3] > 0)
            )
        else:  # RGB
        ...
        trimmed_boxes = [
            box for box in trimmed_boxes if box[2] > box[0] and box[3] > box[1]
        ]
    ...
    def resize_image_and_boxes(self, image, boxes, mask, extra_size=0):
        ...
        resized_image, resized_mask, scaled_boxes = (
            ImageProcessor.resize_to_target_font_size(
                np.array(resized_image_pil),
                np.array(resized_mask_pil),
                shifted_boxes,
                target_height=40,
            )
        )
    ...
    def check_mask_overlap(self, original_mask, processed_image, boxes, threshold=0.30):
        ...
        for box in boxes:
            ...
            if (
                x1 < 0
                or y1 < 0
                or x2 > original_mask_np.shape[1]
                or y2 > original_mask_np.shape[0]
            ):
                return False, processed_image, boxes, original_mask
            ...
            cleaned_mask = cv2.morphologyEx(
                mask_roi, cv2.MORPH_OPEN, kernel, iterations=1
            )
            cleaned_mask = np.where(cleaned_mask > 127, 255, 0).astype(np.uint8)

            _, processed_binary = cv2.threshold(
                processed_roi, 127, 255, cv2.THRESH_BINARY_INV
            )
    ...
    def create_text_image(self, text, font_size, angle_mode="straight", extra_size=0):
        ...
        inter_char_space = (
            [0] * len(text)
            if angle_mode == "plain"
            else [random.randint(0, 20) for _ in text]
        )
    ...
    def save_data(self, image, text, boxes, base_name, is_val=False):
        ...
        if trimmed_boxes:
            for box in trimmed_boxes:
                ...
                if (
                    ...
                    or x1 < 0
                    or y1 < 0
                    or x2 > trimmed_image.width
                    or y2 > trimmed_image.height
                ):
                    return
        ...
        if trimmed_boxes:
            ...
            yolo_boxes = []
            for box in trimmed_boxes:
                yolo_box = self.convert_to_yolo_bbox(
                    box, trimmed_image.width, trimmed_image.height
                )
    ...
    def apply_processing_pipeline(self, image, boxes, mask):
        processed = self.apply_effects(
            np.array(image), np.array(mask), mode=self.args.mode
        )
    ...
    def generate_variants(self, image, text, boxes, mask, base_name, is_val):
        ...
        for process_func in processing_functions:
            ...
            is_valid, cleaned_img, updated_boxes, updated_mask = (
                self.check_mask_overlap(mask, bin_img_pil, boxes)
            )
            if is_valid:
                resized_img, resized_boxes, resized_mask = (
                    self.resize_to_target_dimensions(
        ...
        if len(text) == 1:
            ...
            for morph_func in morph_functions:
                ...
                if morph_func == ImageProcessor.add_text_glow:
                    ...
                    updated_mask_pil = Image.fromarray(updated_mask).convert("L")
                    resized_img, resized_boxes, resized_mask = (
                        self.resize_image_and_boxes(
                            morph_img_pil,
                            expanded_boxes,
                            updated_mask_pil,
                            extra_size=glow_size,
                        )
                    )
                else:
                    morph_img, morph_name = morph_func(image_np)
                    extra_size = 0
                    if morph_name.startswith("Dilation_k"):
                ...
                for process_func in processing_functions:
                    ...
                    bin_img_pil = Image.fromarray(bin_img)
                    is_valid, cleaned_img, updated_boxes, updated_mask = (
                        self.check_mask_overlap(
                            resized_mask, bin_img_pil, resized_boxes
                        )
                    )
                    if is_valid:
                        resized_img, resized_boxes, resized_mask = (
    ...
    def generate_text_char(self, char, counter, size, image, boxes, mask, extra_size=0):
        ...
        for angle in self.rotation_angles:
            ...
            rotated_img, rotated_mask, trimmed_boxes, trim_bbox = (
                self.trim_transparent_pixels(rotated_img, rotated_mask, [box])
            )
            ...
            for angle in self.affine_angles:
                for scale_x in self.scale_x_values:
                    ...
                    (
                        ...
                        transformed_boxes,
                        transform_name,
                    ) = ImageProcessor.apply_affine_transform(
                        image_np, mask_np, [box], angle, scale_x
                    )
                    transformed_img_pil = Image.fromarray(transformed_img).convert(
                        "RGB"
                    )
                    transformed_mask_pil = Image.fromarray(transformed_mask).convert(
                        "L"
                    )
                    ...
                    if "Glow_k" in transform_name:
                        ...
                        transformed_boxes = [
                            ...
                            for x1, y1, x2, y2 in transformed_boxes
                        ]

                    scaled_img, scaled_boxes, scaled_mask = self.resize_image_and_boxes(
                        transformed_img_pil,
                        transformed_boxes,
                        transformed_mask_pil,
    ...
    def generate_text_line(self, line, size, mode, counter, pbar, extra_size=0):
        try:
            if len(line) == 1:
                img, boxes, mask = self.create_text_image(
                    line, size, "plain", extra_size
                )
                self.generate_text_char(
                    line, counter, size, img, boxes, mask, extra_size
                )
            else:
                img, boxes, mask = self.create_text_image(line, size, mode, extra_size)

                bg = self.get_random_background(img.width, img.height)
                bg_pil = Image.fromarray(cv2.cvtColor(bg, cv2.COLOR_BGR2RGB)).convert(
                    "L"
                )
                ...
                plain_img, plain_boxes, plain_mask = self.create_text_image(
                    line, size, "plain", extra_size
                )
                scaled_plain_img, scaled_plain_boxes, scaled_plain_mask = (
                    self.resize_image_and_boxes(
            ...
            logging.error(
                f"Error processing line '{line}' with size {size}, mode {mode}, counter {counter}: {str(e)}"
            )
            raise

    def process_and_save(self, img, text, boxes, mask, base_name, is_val):
        processed_img, processed_mask, processed_boxes = self.apply_processing_pipeline(
            img, boxes, mask
        )
        ...
        self.generate_variants(
            resized_img, text, resized_boxes, resized_mask, base_name, is_val
        )
